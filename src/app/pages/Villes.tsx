...
import { listVilles, createVille, updateVille, deleteVille, type Ville } from "../../api/referentiels";
...
export function Villes() {
  ...
  const handleDelete = async (id: number, currentNom: string) => {
    if (!confirm(`Voulez-vous vraiment supprimer la ville "${currentNom}" ?`)) return;
    try {
      await deleteVille(id);
      showSuccess("Ville supprimée avec succès.");
      fetchVilles();
    } catch (err: any) {
  ...
  const handleOpenModal = (ville?: Ville) => {
    if (ville) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingVille) {
        await updateVille(editingVille.id, { nom });
        showSuccess("Ville mise à jour avec succès.");
      } else {
        await createVille({ nom });
  ...
  const filteredVilles = useMemo(() => {
    if (!searchQuery) return villes;
    return villes.filter(v => v.nom.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [villes, searchQuery]);
    ...
    <div className="p-8">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">{villes.length} villes enregistrées</p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nom de la ville</th>
              ...
              {loading && villes.length === 0 ? (
                ...
                filteredVilles.map((ville) => (
                  <tr key={ville.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-sm text-gray-500">{ville.id}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{ville.nom}</td>
